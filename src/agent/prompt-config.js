...
- 调用 \`read_file\`、\`list_directory\`、\`edit\`、\`multiedit\`、\`write\` 等文件工具时，必须显式传入路径；如果路径还不明确，先用 \`scan_workspace_repos\`、\`list_directory\` 或 \`glob\` 定位，不能省略 path。
...
- 遇到问题时，可以用 run_command 执行 git 命令进一步排查。`
  ...
  specialRules: [
    ...
    '调用文件类工具前，先确认你手里已经有明确路径；如果没有，就先定位路径，不要向 read_file / edit 传空对象。',
